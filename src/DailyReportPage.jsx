...
import {
  ...
  SERVICE_TYPES_EXPECTATION, WEBHOOK_URL,DEAL_RESULTS,
} from "./constants";
    ...
    {showSuccess && (
      ...
      <div style={{ padding: "28px 16px 0" }}>
        ...
        <SectionCard
          ...
          description="سجّل كل الديلات التي لا تزال قيد التفاوض. اضغط ➕ لإضافة ديل، واضغط 🗑 لحذفه. بعض الحقول (كود الموظف، CRM) يكملها المدير لاحقاً."
        >
          <DynamicTable
            rows={form.openDeals}
        ...
        <SectionCard
          icon="✅"
          title="ديلات تم إغلاقها اليوم"
          description="سجّل كل الديلات التي انتهت اليوم سواء بـ Won ✅ أو Lost ❌. اضغط ➕ لإضافة، و🗑 للحذف. حقل سبب الخسارة يظهر فقط عند اختيار Lost."
          accent
        >
          <DynamicTable
            rows={form.closedDeals}
            onAdd={form.addClosedDeal}
            onRemove={form.removeClosedDeal}
            addLabel="إضافة ديل مغلق"
            renderRow={(row, i) => (
              <>
                <div data-error={!!e(`cd_name_${i}`)}>
                  <FormField
                    label="اسم العميل"
                    value={row.client_name}
                    onChange={v => form.updateClosedDeal(i, "client_name", v)}
                    error={e(`cd_name_${i}`)}
                    placeholder="اسم العميل"
                    required
                  />
                </div>
                <div data-error={!!e(`cd_phone_${i}`)}>
                  <FormField
                    label="رقم الهاتف"
                    value={row.client_phone}
                    onChange={v => form.updateClosedDeal(i, "client_phone", v)}
                    error={e(`cd_phone_${i}`)}
                    placeholder="966551234567"
                    required
                  />
                </div>
                <div data-error={!!e(`cd_service_${i}`)}>
                  <FormField
                    label="الخدمة"
                    value={row.service}
                    onChange={v => form.updateClosedDeal(i, "service", v)}
                    error={e(`cd_service_${i}`)}
                    placeholder="مثال: كتاب 120 صفحة"
                    required
                  />
                </div>
                <div data-error={!!e(`cd_result_${i}`)}>
                  <FormField
                    label="النتيجة"
                    type="select"
                    value={row.result}
                    onChange={v => form.updateClosedDeal(i, "result", v)}
                    options={DEAL_RESULTS}
                    error={e(`cd_result_${i}`)}
                    required
                  />
                </div>
                {/* سبب الخسارة — يظهر فقط لو Lost */}
                {row.result === "Lost" && (
                  <div style={{ gridColumn: "1 / -1" }} data-error={!!e(`cd_reason_${i}`)}>
                    <FormField
                      label="سبب الخسارة"
                      value={row.loss_reason}
                      onChange={v => form.updateClosedDeal(i, "loss_reason", v)}
                      error={e(`cd_reason_${i}`)}
                      placeholder="لماذا لم يتم إغلاق الديل؟"
                      required
                    />
                  </div>
                )}
